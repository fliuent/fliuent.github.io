...
export default function HomeSection({ profile, projects, copy }: HomeSectionProps) {
  ...
  return (
    ...
    >
      ...
      {/* Personal Projects Showcase Section directly from Image 3 */}
                ...
                {proj.keyFeatures && (
                  <div className="text-xs md:text-sm font-sans pt-1">
                    <p className="text-gray-500 font-light">
                      <span className="font-bold text-[#007bfe] mr-1.5">Key Features:</span>
                      {proj.keyFeatures}
                    </p>
                  </div>
                )}
      ...
      <section className="bg-white rounded-2xl p-6 md:p-8 border border-gray-100 shadow-xs" id="collaboration-board">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <form onSubmit={handleSubmitMessage} className="lg:col-span-6 space-y-4">
            <div>
              <label htmlFor="guest-name" className="block text-xs font-semibold text-gray-600 mb-1">Your Name *</label>
              <input
                id="guest-name"
                type="text"
                required
            ...
            <div>
              <label htmlFor="guest-email" className="block text-xs font-semibold text-gray-600 mb-1">Your Email (Optional)</label>
              <input
                id="guest-email"
                type="email"
                value={email}
            ...
            <div>
              <label htmlFor="guest-msg" className="block text-xs font-semibold text-gray-600 mb-1">Message Detail *</label>
              <textarea
                id="guest-msg"
                required
                rows={4}
                ...
                placeholder="Leave notes, feedback, or general questions..."
                className="w-full px-3 py-2 text-sm bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:border-[#007bfe] focus:bg-white transition-all"
              />
            </div>
            ...
            {submitted && (
              <div className="flex items-center gap-2 text-xs text-emerald-600 bg-emerald-50 px-3 py-2.5 rounded-lg border border-emerald-100">
                ...
                <span>Thank you! Your message was saved locally in guestbook.</span>
              </div>
            )}
          </form>
          ...
          <div className="lg:col-span-6 space-y-4">
            <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider flex items-center gap-2">
              ...
              <span>Submitted Notes ({messages.length})</span>
            </h3>

            {messages.length === 0 ? (
              <div className="text-center py-10 px-4 border border-dashed border-gray-100 rounded-xl">
                <p className="text-gray-400 text-xs">No local guest records yet.</p>
                <p className="text-[10px] text-gray-300 mt-1">Be the first to submit a custom note above!</p>
              </div>
            ) : (
