...
export default function TeachingSection({ teaching }: TeachingSectionProps) {
  ...
  const { teachingSection } = siteContent;
    ...
    >
            ...
            <thead>
              <tr className="border-b border-gray-100 bg-[#fafafc]">
                <th className="px-5 py-4 text-xs font-bold font-sans text-gray-600 uppercase tracking-wider min-w-[240px]">
                  {teachingSection.tableHeaders.course}
                </th>
                <th className="px-5 py-4 text-xs font-bold font-sans text-gray-600 uppercase tracking-wider">
                  {teachingSection.tableHeaders.instructor}
                </th>
                <th className="px-5 py-4 text-xs font-bold font-sans text-gray-600 uppercase tracking-wider">
                  {teachingSection.tableHeaders.position}
                </th>
                <th className="px-5 py-4 text-xs font-bold font-sans text-gray-600 uppercase tracking-wider">
                  {teachingSection.tableHeaders.university}
                </th>
                <th className="px-5 py-4 text-xs font-bold font-sans text-gray-600 uppercase tracking-wider">
                  {teachingSection.tableHeaders.time}
                </th>
              </tr>
            </thead>
              ...
              {teaching.map((row) => (
                ...
                >
                  ...
                  <td className="px-5 py-4 font-sans text-gray-600">
                    {row.instructor || teachingSection.emptyInstructor}
                  </td>
                  
                  {/* {teachingSection.tableHeaders.position} Badge */}
                  ...
                  {/* {teachingSection.tableHeaders.time} / Period */}
      ...
      <div className="flex flex-wrap items-center gap-3 pt-3">
        ...
        >
          ...
          <span>{teachingSection.dutiesButtonLabel}</span>
        </button>
        ...
        >
          ...
          <span>{teachingSection.evaluationButtonLabel}</span>
        </button>
      </div>
          ...
          >
            ...
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <div className="flex items-center gap-2">
                <span className="p-1 px-2 bg-indigo-500/20 text-indigo-300 rounded-md text-[10px] font-mono font-bold uppercase tracking-wider">
                  {activeDialog === "duties" ? teachingSection.dutiesBadge : teachingSection.evaluationBadge}
                </span>
                <h4 className="font-heading text-sm sm:text-base font-bold text-white">
                  {activeDialog === "duties" ? teachingSection.dutiesTitle : teachingSection.evaluationTitle}
                </h4>
              </div>
              <button
                onClick={() => setActiveDialog(null)}
                className="p-1.5 text-slate-400 hover:text-white hover:bg-slate-800 rounded-full transition-colors cursor-pointer"
                aria-label={teachingSection.closePanelLabel}
              >
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            {activeDialog === "duties" ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-1">
                {teachingSection.duties.map((item, idx) => (
                  <div key={idx} className="space-y-2">
                    <span className="text-xs font-mono font-bold text-blue-400 flex items-center gap-1">
                      <GraduationCap className="w-3.5 h-3.5 shrink-0" />
                      <span>{item.univ} {teachingSection.dutiesResponsibilitySuffix}</span>
                    </span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-1">
                {teachingSection.evaluations.map((item, idx) => (
