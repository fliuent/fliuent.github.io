...
export default function EducationSection({ degrees, courseCategories, awards }: EducationSectionProps) {
  const { educationSection, placeholders } = siteContent;
  ...
  return (
    ...
    >
      ...
      <section id="academic-degrees" className="space-y-6">
        <div className="flex items-center gap-3 border-b border-gray-100 pb-3">
          ...
          <h2 className="font-heading text-2xl font-bold text-[#007bfe]" id="edu-title">
            {educationSection.educationTitle}
          </h2>
        </div>
            ...
            <div key={degree.id} className="group relative flex flex-col gap-1 hover:translate-x-1 transition-all duration-300">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 sm:gap-4">
                <p className="text-sm text-gray-600 font-sans">
                  {degree.degreeName} {educationSection.degreeConnector} <span className="font-medium text-gray-700">{degree.field}</span>
                  {degree.gpa && (
                    <>
                      , <span className="text-xs font-semibold text-blue-600 font-mono">{educationSection.gpaLabel} {degree.gpa}</span>
                    </>
                  )}
                  {degree.supervisor && (
                    <>
                      {" "}({educationSection.supervisorLabel}{" "}
                      {degree.supervisorUrl ? (
                        <a
                          href={degree.supervisorUrl}
                          target="_blank"
      ...
      <section id="academic-courses" className="space-y-6">
        <div className="flex items-center gap-3 border-b border-gray-100 pb-3">
          ...
          <h2 className="font-heading text-xl md:text-2xl font-bold text-[#007bfe]" id="courses-title">
            {educationSection.coursesTitle}
          </h2>
        </div>

        <div className="space-y-8">
          {courseCategories.map((category) => {
            const isTbdOnly = category.courses.length === 1 && category.courses[0].title === placeholders.toBeDone;
            return (
                ...
                {isTbdOnly ? (
                  ...
                  <div className="divide-y divide-gray-100/65">
                    {category.courses.map((course) => (
                      ...
                      >
                        {/* Course Title and Code with inline Links formatted as ({educationSection.certificateLabel}) / ({educationSection.githubRepoLabel}) */}
                          ...
                          <span className="inline-flex gap-1.5 ml-2 font-mono text-[11px]">
                            {course.certificateUrl && (
                              <span className="text-gray-400">
                                ...
                                >
                                  {educationSection.certificateLabel}
                                </a>
                                )
                              </span>
                            )}
                            {course.githubUrl && (
                              <span className="text-gray-400">
                                ...
                                <a
                                  ...
                                  className="text-[#007bfe] hover:underline inline-flex items-center gap-0.5"
                                >
                                  {educationSection.githubRepoLabel}
                                </a>
                                )
                              </span>
                            )}
      ...
      {/* 3. Selected {educationSection.awardsTitle} Section */}
