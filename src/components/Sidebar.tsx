...
export default function Sidebar({
  ...
  const iconMap = { User, GraduationCap, BookOpen, Heart };
  const menuItems = siteContent.navigation.map((item) => ({
    ...item,
    icon: iconMap[item.iconName as keyof typeof iconMap] || User,
  }));
  const { profile: profileContent, footer, ui } = siteContent;
      ...
      <header className="md:hidden fixed top-0 left-0 right-0 h-16 bg-white border-b border-gray-100 px-4 flex items-center justify-between z-20" id="mobile-header">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            title={ui.toggleCustomizerTitle}
          >
      ...
      >
        ...
        <div className="space-y-4">
          ...
          >
            ...
            <span>{customizing ? ui.closeCustomizerLabel : ui.customizeContentLabel}</span>
          </button>

          <div className="text-center font-mono text-[10px] text-gray-400">
            <p>© {new Date().getFullYear()} {profile.name}</p>
            <p className="mt-0.5">{footer.lastUpdated}</p>
          </div>
        </div>
      </aside>
    </>
